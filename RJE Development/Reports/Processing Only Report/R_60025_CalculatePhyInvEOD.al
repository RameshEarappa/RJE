report 60025 "Calculate Inventory EOD"
{
    Caption = 'Calculate Inventory EOD';
    ProcessingOnly = true;
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            begin
                ...
                //LT07012021
                CLEAR(RecLocation);
                IF RecLocation.GET(Item.GetFilter("Location Filter")) THEN
                    IF NOT RecLocation."DR Location" THEN
                        ERROR('Location Should be DR Location');
                //LT07012021
                if ItemJnlTemplate.Get(ItemJnlLine."Journal Template Name") then;
                if ItemJnlBatch.Get(ItemJnlLine."Journal Template Name", ItemJnlLine."Journal Batch Name") then;
    ...
    var
        ...
        UnloadStockEntryNumber: Integer;//Added on 15APR2021
    ...
    begin
        ...
        if (Quantity2 <> 0) or ZeroQty then begin
            ...
            //LT05012021
            //ItemJnlLine."Qty. (Phys. Inventory)" := 0;//15APR2021
            //15APR2021
            ItemJnlLine."Phys. Inventory" := true;
            if UnloadStockEntryNumber <> 0 then
                ItemJnlLine.Validate("Qty. (Phys. Inventory)", GetPhysicalQuantity(ItemNo))////Added validate to mark reconciled flag true
            else
                ItemJnlLine.Validate("Qty. (Phys. Inventory)", 0);//Added validate to mark reconciled flag true
            //15APR2021
            ItemJnlLine.Time := Time;
            //ItemJnlLine.Reconciled := FALSE;
            //LT05012021
            ...
            IF ItemJnlLine."Qty. (Calculated)" = ItemJnlLine."Qty. (Phys. Inventory)" THEN
                ItemJnlLine.Reconciled := TRUE
            ELSE
                ItemJnlLine.Reconciled := FALSE;
    ...
    //15APR2021-Krishna- Created fn to fetch Qty physical from Unload Stock
    local procedure GetPhysicalQuantity(ItemNumber: code[20]): Decimal
    var
        RecUnloadStockLine: Record "Transfer Order Unload Line";
    begin
        Clear(RecUnloadStockLine);
        RecUnloadStockLine.SetRange("Transfer Order Entry No.", UnloadStockEntryNumber);
        RecUnloadStockLine.SetRange("Item No.", ItemNumber);
        if RecUnloadStockLine.FindFirst() then
            exit(RecUnloadStockLine.Quantity)
        else
            exit(0);
    end;

    procedure SetUnloadStockFilters(UnloadStockEntryNo: Integer)
    begin
        UnloadStockEntryNumber := UnloadStockEntryNo;
    end;
